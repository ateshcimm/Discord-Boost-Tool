...
from config import tkn, secret, client_id, redirect
...
guild = input("Id Server For Boost | ")
nickname = input("Name For Tokens (not anyway) | ")
...
def rename(tk):
    ...
    if r.status_code in (200, 201, 204):
        print(Fore.GREEN +"[ ateshcim ] Nice Name | %s" % (nickname))
        return "ok"
    else:
        print(Fore.RED +"[ ateshcim ] Falid Nick Name Error")
        return "error"
...
def authorizer(tk):
    ...
    if r.status_code in (200, 201, 204):
        ...
        print("[ ateshcim ] This Token Run")
        access_token = exchange['access_token']
        userid = get_user(access_token)
        add_to_guild(access_token, userid)
        print(Fore.GREEN + "[ ateshcim ] This Token Join Server | %s" % (guild))
        if not nickname == "":
...
def main(tk):
    ...
    for x in idk:
        ...
        if r.status_code in (200, 201, 204):
            print(Fore.MAGENTA + "[ ateshcim ] This Token Is Boosting %s" % (guild))

f = open("python/token.txt", "r").readlines()
os.system("cls" if os.name == "nt" else "clear")
for tk in f:
    tk = tk.strip()
